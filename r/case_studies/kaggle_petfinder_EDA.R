...
# read all data files
train<- read_csv("kaggle-petfinder/data/train.csv")
test<- read_csv("kaggle-petfinder/data/test.csv")
...
p +geom_bar(stat = "count", color="black",aes(fill = Type))+
  mytheme()+
  ggtitle("(A) School location")+
  scale_x_discrete(name="School Type", labels=c("Boys","Girls","Co-educational"))+
  scale_y_continuous(name = "Enroll count")+
  scale_color_manual(values = mycolors)+
  scale_fill_discrete(name = "Rural/Urban")

# pick some cols at random for further analysis
somecols<- train[,c(1,3:18,20,24)]
