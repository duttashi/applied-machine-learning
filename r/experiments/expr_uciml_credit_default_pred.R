...
# Script last modified date: 29/9/2020
...
# 1. rename the dependent variable and coerce it to factor
...
# assign smoted_data dataframe to variable df
# df<- smoted_data
...
# Variables with skew value greater than 1 are skewed and should be treated
# variables BILL_AMT1 to PAY_AMT6 are psotively skewed
...
names(df)
df.norm<- df[,c(10:24,1:9,25)]
df.norm<- df
names(df.norm)
df.norm[,12:23]<- log(df.norm[12:23],2) # where 2 is log base 2

# Check for skewed variables again
skewedVars<- NA
for(i in names(df.norm)){
  if(is.numeric(df.norm[,i])){
    if(i != "income"){
      # Enters this block if variable is non-categorical
      skewVal <- skewness(df.norm[,i])
      print(paste(i, skewVal, sep = ": "))
      if(abs(skewVal) > 0.5){
        skewedVars <- c(skewedVars, i)
      }
